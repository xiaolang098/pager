...
function usergroup_cache() {
    ...
    $usergroup = $_SGLOBAL['groupname'] = array();
    //$highest = true;
    //$lower = '';
    $query = $_SGLOBAL['db']->query('SELECT * FROM '.tname('usergroup')." ORDER BY gid ASC");
    while (($group = $_SGLOBAL['db']->fetch_array($query)) != false) {
        /*
        ...
        */
        ...
        $_SGLOBAL['groupname'][$group['gid']] = array(
    ...
    //生成缓存文件
    cache_write('usergroup', "_SGLOBAL['groupname']", $_SGLOBAL['groupname']);
...
//更新文章分类缓存
function articletype_cache(){
    global $_SGLOBAL;

    $_SGLOBAL['articletype'] = array();
    ...
    cache_write('articletype',"_SGLOBAL['articletype']",$_SGLOBAL['articletype']);
}
