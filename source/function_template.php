...
/**
 * 【极客】模板函数
 * $Id: function_template.php 27 2010-09-25 11:33:10Z wukai$
 */
...
function parse_template($tpl) {
    ...
    //附加处理
    if(URL_REWRITE){
        $url_rewrite = "<?php ob_out();?>";
    }else{
        $url_rewrite = "";
    }
    $template = "<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('".implode('|', $_SGLOBAL['sub_tpls'])."', '$_SGLOBAL[timestamp]', '$tpl');?>$template$url_rewrite";
...
function readtemplate($name) {
    ...
    $tpl = strexists($name, '/') ? $name : "template/$name";
    $tplfile = S_ROOT . './' . $tpl . '.htm';
